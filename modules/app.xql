...
declare function app:registryLetters($node as node(), $model as map(*)) {
    ...
    let $lettersReceiver := for $letter in $letters
                        ...
                        return
                            (<div receiver="{$persons[@xml:id=$correspReceivedId]//tei:titleStmt/tei:title}" receiverId="{$correspReceivedId}" count="{count($letterEntry)}" xmlns="http://www.w3.org/1999/xhtml">
                                {for $each in $letterEntry
                                    order by $each
                                    return
                                        $each}
    ...
    let $lettersSender := for $letter in $letters
                        ...
                        return
                            (<div sender="{$persons[@xml:id=$correspSentId]//tei:titleStmt/tei:title}" senderId="{$correspSentId}" count="{count($letterEntry)}" xmlns="http://www.w3.org/1999/xhtml">
                                {for $each in $letterEntry
                                    order by $each
                                    return
                                        $each}
